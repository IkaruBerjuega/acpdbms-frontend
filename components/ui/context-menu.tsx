...
interface ReusableCxtMenuProps {
  menuLabel: string;
      ...
      <ContextMenuContent className="w-48">
        ...
        {items.map((item, index) => (
          <React.Fragment key={index}>
            {item.isDialog ? (
              <Dialog
                open={selectedDialog === item.actionName}
                onOpenChange={(open) =>
                  setSelectedDialog(open ? item.actionName : null)
                }
              >
              ...
              <ContextMenuItem
                ...
                className="h-7 px-2 flex justify-start items-center gap-2 cursor-pointer"
              >
                <Image
                  src={item.btnSrc}
                  alt={`${item.actionName} button`}
                  width={16}
                  height={16}
                />
                {item.actionName}
