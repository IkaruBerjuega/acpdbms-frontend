...
export default function FirstLogin() {
  ...
  const { getAuthenticatedUser, updateProfile } = useProfile();
  ...
  const processForm: SubmitHandler<firstLogin> = async (data) => {
    ...
    const isSuccessful = await updateProfile(data);
    ...
    if (isSuccessful !== undefined) {
      ...
      toast({
        variant: 'destructive',
        title: 'Notification',
        description: "Can't Update Profile Information",
      });
    }
  };
          ...
          <div className='flex flex-col items-start gap-2 text-3xl lg:text-4xl lg:mt-24 lg:ml-12'>
            ...
            {step === 0 ? (
              <p className='text-primary font-semibold'>
                Welcome to Larry's Home Designs. Click proceed when you're
                ready.
              </p>
            ) : step === 1 ? (
              <p className='text-primary font-semibold'>
